...
const ViewDetails = ({ patient, onClose }) => {
  ...
  const PaymentCard = ({ payment }) => (
    <div className="bg-gray-50 rounded-lg p-4 mb-3">
      <div className="flex justify-between items-start mb-2">
        <div>
          <h4 className="font-medium text-gray-900">Paiement le {payment.date}</h4>
          <p className="text-sm text-gray-500">Reçu : {payment.receiptNumber}</p>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
      ...
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">{patient.name}</h2>
              <p className="text-sm text-gray-500">Détails du patient</p>
            </div>
          </div>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700">
            <X className="w-6 h-6" />
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          {activeTab === "overview" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <InfoRow icon={User} label="Nom complet" value={patient.name} />
                <InfoRow icon={Phone} label="Numéro de téléphone" value={patient.phone} />
                <InfoRow icon={Calendar} label="Enregistré le" value={formatDate(patient.createdAt)} />
              </div>
          ...
          {activeTab === "financial" && (
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">Résumé financier</h3>
              ...
              {patient.payments && patient.payments.length > 0 ? (
                ...
                <p className="text-sm text-gray-500">Aucun historique de paiement disponible.</p>
              )}
            </div>
          )}
