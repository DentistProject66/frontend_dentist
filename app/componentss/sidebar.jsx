...
const Sidebar = () => {
  ...
  const data = [
    ...
    { name: "Patients", id: 2, link: "/Patients", icon: <User size={20} /> },
    { name: "Rendez-vous", id: 3, link: "/appointments", icon: <CalendarDays size={20} /> },
    { name: "Gestion de Paiement", id: 4, link: "/payment", icon: <DollarSign size={20} /> },
    { name: "Archives", id: 5, link: "/archives", icon: <Archive size={20} /> },
    { name: "Profil", id: 6, link: "/profile", icon: <UserCircle size={20} /> },
  ];
  ...
  const handleItemClick = (id, link) => {
    ...
    try {
      ...
      console.error("Navigation error:", error);
      alert("Échec de navigation. Vérifiez si la route existe.");
    }
  };
  ...
  const handleLogout = () => {
    ...
    router.push("/sign_in"); // redirige vers sign_in
  };

  // Filtrer les items selon le rôle
  const filteredData =
    user?.role === "assistant"
      ? data.filter((item) => item.id !== 4) // Cache "Paiement" pour assistant
      : data;
    ...
    <div className="w-64 bg-white h-screen border-r border-gray-200 shadow-sm flex flex-col">
      ...
      <div className="p-4 border-t border-gray-100 space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center justify-center px-4 py-2 text-red-600 rounded-lg hover:bg-red-50 transition-colors"
        >
