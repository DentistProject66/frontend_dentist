...
const InPaymentTable = ({ token }) => {
  ...
  // Handle payment update - Updated to modify local state instead of refetching
  const handlePaymentUpdated = (updatedPayment, newAmountPaid) => {
    ...
    setPaymentsData(prevPayments => 
      prevPayments.map(payment => 
        payment.id === updatedPayment.id 
          ? {
              ...payment,
              amount_paid: (parseFloat(payment.amount_paid) + parseFloat(newAmountPaid)).toFixed(2),
              remaining_balance: (parseFloat(payment.remaining_balance) - parseFloat(newAmountPaid)).toFixed(2),
              payment_date: new Date().toISOString().split('T')[0] // Update to today's date
            }
          : payment
      ).filter(payment => parseFloat(payment.remaining_balance) > 0) // Remove if fully paid
    );
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 w-full p-5 max-w-6xl mx-auto">
      ...
      {isEditModalOpen && selectedPayment && (
        <EditPayment
          ...
          patientId={selectedPayment.patient_id}
          onClose={() => setIsEditModalOpen(false)}
          onPaymentUpdated={(payment, newAmount) => handlePaymentUpdated(payment, newAmount)}
        />
      )}
    </div>
  );
